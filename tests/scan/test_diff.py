...
class TestExplicitChangedPaths:
    ...
    def test_changed_paths_takes_priority_over_base_ref(self, tmp_path: Path) -> None:
        ...
        with patch("subprocess.run") as mock_run:
            result = resolve_diff_target(tmp_path, base_ref="HEAD~1", changed_paths=["a.py"])
...
class TestGitDiff:
    ...
    def test_returns_changed_files_from_git(self, tmp_path: Path) -> None:
        ...
        with patch("subprocess.run", return_value=self._make_git_proc("changed.py\n")):
    ...
    def test_skips_deleted_files_from_git(self, tmp_path: Path) -> None:
        with patch("subprocess.run", return_value=self._make_git_proc("deleted_file.py\n")):
            result = resolve_diff_target(tmp_path, base_ref="main")
        assert result == []

    def test_git_nonzero_exit_raises(self, tmp_path: Path) -> None:
        proc = MagicMock(returncode=128, stdout="", stderr="fatal: bad revision")
        with patch("subprocess.run", return_value=proc), pytest.raises(
            DiffResolutionError, match="git diff failed"
        ):
    ...
    def test_git_not_found_raises(self, tmp_path: Path) -> None:
        with patch("subprocess.run", side_effect=FileNotFoundError), pytest.raises(
            DiffResolutionError, match="git binary not found"
        ):
