...
from compliance_mcp.models import Severity
...
class TestDedupe:
    ...
    def test_first_occurrence_kept_on_duplicate(self) -> None:
        f1 = make_finding(title="Dup", file_path="foo.py", source="lens-a")
        f2 = make_finding(title="Dup", file_path="foo.py", source="lens-b")
        result = normalize_findings([f1, f2])
        assert result[0].source == "lens-a"
...
class TestOrdering:
    ...
    def test_none_file_path_sorts_before_named(self) -> None:
        findings = [
            make_finding(title="T1", severity=Severity.HIGH, file_path="b.py"),
            make_finding(title="T2", severity=Severity.HIGH, file_path=None),
        ]
        result = normalize_findings(findings)
        # empty string sorts before "b.py"
