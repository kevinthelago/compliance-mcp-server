...
class TestScanProjectFindings:
    ...
    def test_findings_are_normalised(self, tmp_path: Path) -> None:
        """Duplicate fingerprints must be deduplicated in the response."""
...
class TestScanDiffExplicitPaths:
    ...
    def test_findings_with_no_file_path_included(self, tmp_path: Path) -> None:
        (tmp_path / "a.py").write_text("x")
        f_no_path = make_finding(title="no-path", file_path=None)
        lens = StubLens(name="sec", findings=[f_no_path])
        with _patch_lenses([lens]):
            result = scan_diff(str(tmp_path), changed_paths=["a.py"])
        titles = [f["title"] for f in result["findings"]]
        assert "no-path" in titles
